...
class UpdateIRSRequest extends FormRequest
{
    ...
    {
        $irsId = $this->route('internal_revenue_service') ?? $this->route('id');
        
        return [
            'irs_name' => 'sometimes|required|string|max:255',
            'short_name' => 'sometimes|required|string|max:50',
            'state_id' => [
                'sometimes',
                'required',
                'exists:states,id',
                Rule::unique('internal_revenue_services', 'state_id')->ignore($irsId)
            ],
